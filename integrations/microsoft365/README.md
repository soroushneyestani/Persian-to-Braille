...
The Word add-in now includes an add-in only XML manifest, a Home-ribbon
`Translate Selection` command, and a browser task pane.
...
```text
pnpm --filter @persian-braille/microsoft365 run preview
```

The preview command expects a trusted localhost certificate generated by
`office-addin-dev-certs`. Certificate setup and Office sideloading are completed
in Phase 9.5.

The task pane loads Office.js from the Microsoft CDN and keeps translation behind
the existing SDK boundary.
